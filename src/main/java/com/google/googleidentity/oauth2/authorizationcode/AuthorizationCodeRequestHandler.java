        ...
        if (oauth2Request.getRequestBody().getResponseType()
            ...
            try {
                ...
                if (!oauth2Request.getAuthorizationResponse().getState().isEmpty()) {
                    uriBuilder.addParameter(
                            ...
                            oauth2Request.getAuthorizationResponse().getState());
                }
            ...
            if (!opRequest.get().getRequestAuth().getClientId()
                ...
                throw new InvalidGrantException(
                        InvalidGrantException.ErrorCode.CODE_CLIENT_MISMATCH);
            }
            ...
            if (!redirectUri.equals(opRequest.get().getAuthorizationResponse().getRedirectUri())) {
                throw new InvalidGrantException(
                        InvalidGrantException.ErrorCode.CODE_REDIRECT_URI_MISMATCH);
            }
            ...
            json.appendField(
                    "expires_in",
                    token.getExpiredTime() - Instant.now().getEpochSecond());
