...
public final class AuthorizationCodeService {
  ...
  private static final String AUTH_CODE_LENGTH =
      System.getenv("AUTH_CODE_LENGTH") ==
          null ? "10" : System.getenv("AUTH_CODE_LENGTH");
  ...
  /**
   * Set the byteLength as the minimum one we need for BaseEncoding.base64Url to generate a string
   * with length of codeLength. A char in base64Url need 6 bits (2^6 = 64) and a byte has 8 bits. So
   * byteLength = (codeLength * 6 - 1) / 8 + 1;
   */
  ...
  /**
   * Generate a authorization code for the request, always success. Because once a duplicate code
   * generated, we will try another one.
   *
  ...
  public String getCodeForRequest(OAuth2Request request) {
    ...
    String code =
        generateCode(
            request.getRequestAuth().getClientId(), request.getRequestAuth().getUsername());
  ...
  /**
   * Associate with client and username to reduce collisions. When generating random bytes, set
   * first numPrefixBytesToCopy bytes as the first numPrefixBytesToCopy bytes of sha256(clientID +
   * username). To get enough randomness, numPrefixBytesToCopy = min(3, byteLength/2).
   */
  private String generateCode(String clientID, String username) {
    ...
    byte[] prefixBytes =
        Hashing.sha256().hashString(clientID + username, Charsets.UTF_8).asBytes();
